...
void kInitializePageTables(void){
    ...
    kSetPageEntryData(&(pstPML4TEntry[0]), 0x00, 0x101000, PAGE_FLAGS_DEFAULT, 0x0);
    for(i = 1; i < PAGE_MAXENTRYCOUNT ; i++){
        kSetPageEntryData(&(pstPML4TEntry[i]), 0x0, 0x0, 0x0, 0x0);
    }
    ...
    for(i = 64; i < PAGE_MAXENTRYCOUNT; i++){
        kSetPageEntryData(&(pstPDPTEntry[i]), 0x0, 0x0, 0x0, 0x0);
    }
