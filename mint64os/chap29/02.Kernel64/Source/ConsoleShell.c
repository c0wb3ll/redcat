...
void kExecuteCommand( const char* pcCommandBuffer ) {
    ...
    for( iSpaceIndex = 0; iSpaceIndex < iCommandBufferLength; iSpaceIndex++ ) { if( pcCommandBuffer[ iSpaceIndex ] == ' ' ) { break; } }
...
static void kHelp( const char* pcCommand ) {
    ...
    for( i = 0; i < iCount; i++ ) {
        ...
        if( iLength > iMaxCommandLength ) { iMaxCommandLength = iLength; }
...
static void kTestTask2( void ) {
    ...
    while( 1 ) {
        ...
        pstScreen[ iOffset ].bCharactor = vcData[ i % 7 ];
...
static void kCreateTestTask( const char* pcParameterBuffer ) {
    ...
    case 1:
        for( i = 0; i < kAToI( vcCount, 10 ); i++ ) {
            ...
            if( kCreateTask( TASK_FLAGS_LOW | TASK_FLAGS_THREAD, 0, 0, ( QWORD ) kTestTask1 ) == NULL ) { break; }
    ...
    default:
        for( i = 0; i < kAToI( vcCount, 10 ); i++ ) {
            ...
            if( kCreateTask( TASK_FLAGS_LOW | TASK_FLAGS_THREAD, 0, 0, ( QWORD ) kTestTask2 ) == NULL ) { break; }
...
static void kDownloadFile( const char* pcParameterBuffer ) {
    ...
    if( dwReceivedSize != dwDataLength ) {
        ...
        kPrintf( "\nReceived Complete. Total Size [%d] Byte\n", dwReceivedSize );
