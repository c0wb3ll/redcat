...
// MP 설정 테이블을 분석해서 필요한 정보를 저장하는 경우
...
void kPrintMPConfigurationTable( void ) {
    ...
    for( i = 0 ; i < pstMPTableHeader->wEntryCount; i++ ) {
        ...
        case MP_ENTRYTYPE_IOINTERRUPTASSIGNMENT:
            ...
            kPrintf( "  Interrupt Type : 0x%X (%s)\n", pstIOAssignmentEntry->bInterruptType, vpcInterruptType[ pstIOAssignmentEntry->bInterruptType ] );
            kPrintf( "  I/O Interrupt Flags : 0x%X (%s, %s)", pstIOAssignmentEntry->wInterruptFlags, vpcInterruptFlagsPO[ pstIOAssignmentEntry->wInterruptFlags & 0x03 ], vpcInterruptFlagsEL[ ( pstIOAssignmentEntry->wInterruptFlags >> 2 ) & 0x03 ] );
        ...
        case MP_ENTRYTYPE_LOCALINTERRUPTASSIGNMENT:
            ...
            kPrintf( "  Interrupt Type : 0x%X (%s)\n", pstLocalAssignmentEntry->bInterruptType, vpcInterruptType[ pstLocalAssignmentEntry->bInterruptType ] );
            kPrintf( "  I/O Interrupt Flags : 0x%X (%s, %s)\n", pstLocalAssignmentEntry->wInterruptFlags, vpcInterruptFlagsPO[ pstLocalAssignmentEntry->wInterruptFlags & 0x03 ], vpcInterruptFlagsEL[ ( pstLocalAssignmentEntry->wInterruptFlags >> 2 ) & 0x03 ] );
            kPrintf( "  Source BUS ID : %d\n", pstLocalAssignmentEntry->bSourceBUSID );
            kPrintf( "  Source BUS IRQ : %d\n", pstLocalAssignmentEntry->bSourceBUSIRQ );
            kPrintf( "  Desitnation Local APIC ID : %d\n", pstLocalAssignmentEntry->bDestinationLocalAPICID );
            kPrintf( "  Desitnation Local APIC LINTIN : %d\n\n", pstLocalAssignmentEntry->bDestinationLocalAPICLINTIN );
