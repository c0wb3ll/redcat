...
int Main( char* pcArgument ) {
    ...
    DrawText( qwWindowID, 7, 200, RGB( 0, 0, 0 ), RGB( 255, 255, 255 ), pcStartMessage, strlen( pcStartMessage ) );
        ...
        if( ( g_stGameInfo.bGameStart == TRUE ) && ( ( GetTickCount() - qwLastTickCount ) > ( 300 - ( g_stGameInfo.qwLevel * 10 ) ) ) ) {
            ...
            if( IsMovePossible( g_stGameInfo.iBlockX, g_stGameInfo.iBlockY + 1 ) == FALSE ) {
                ...
                if( FreezeBlock( g_stGameInfo.iBlockX, g_stGameInfo.iBlockY ) == FALSE ) {
                    ...
                    DrawText( qwWindowID, 82, 230, RGB( 0, 0, 0 ), RGB( 255, 255, 255 ), "GameOver", 8 );
                    DrawText( qwWindowID, 7, 250, RGB( 0, 0, 0 ), RGB( 255, 255, 255 ), pcStartMessage, strlen( pcStartMessage ) );
...
void Initialize( void ) {
    ...
    g_stGameInfo.vstBlockColor[ 1 ] = RGB( 255, 135, 135 );
    g_stGameInfo.vstBlockColor[ 2 ] = RGB( 135, 255, 135 );
    g_stGameInfo.vstBlockColor[ 3 ] = RGB( 255, 135, 255 );
    g_stGameInfo.vstBlockColor[ 4 ] = RGB( 255, 255, 135 );
    g_stGameInfo.vstBlockColor[ 5 ] = RGB( 135, 255, 255 );
    g_stGameInfo.vstEdgeColor[ 1 ] = RGB( 150, 75, 75 );
    g_stGameInfo.vstEdgeColor[ 2 ] = RGB( 75, 150, 75 );
    g_stGameInfo.vstEdgeColor[ 3 ] = RGB( 150, 75, 150 );
    g_stGameInfo.vstEdgeColor[ 4 ] = RGB( 150, 150, 75 );
    g_stGameInfo.vstEdgeColor[ 5 ] = RGB( 75, 150, 150 );
...
void DrawGameArea( QWORD qwWindowID ) {
    ...
    for( j = 0 ; j < BOARDHEIGHT ; j++ ) {
        ...
        for( i = 0 ; i < BOARDWIDTH ; i++ ) {
            ...
            if( g_stGameInfo.vvbBoard[ j ][ i ] != EMPTYBLOCK ) {
                ...
                DrawRect( qwWindowID, i * BLOCKSIZE, iY + ( j * BLOCKSIZE ), ( i + 1 ) * BLOCKSIZE, iY + ( ( j + 1 ) * BLOCKSIZE ), stColor, TRUE );

                stColor = g_stGameInfo.vstEdgeColor[ g_stGameInfo.vvbBoard[ j ][ i ] ];
                DrawRect( qwWindowID, i * BLOCKSIZE, iY + ( j * BLOCKSIZE ), ( i + 1 ) * BLOCKSIZE, iY + ( ( j + 1 ) * BLOCKSIZE ), stColor, FALSE );
                stColor = g_stGameInfo.vstEdgeColor[ g_stGameInfo.vvbBoard[ j ][ i ] ];
                DrawRect( qwWindowID, i * BLOCKSIZE + 1, iY + ( j * BLOCKSIZE ) + 1, ( i + 1 ) * BLOCKSIZE - 1, iY + ( ( j + 1 ) * BLOCKSIZE ) - 1, stColor, FALSE );
    ...
    DrawRect( qwWindowID, 0, iY, BLOCKSIZE * BOARDWIDTH - 1, iY + BLOCKSIZE * BOARDHEIGHT - 1, RGB( 255, 255, 255 ), FALSE );
