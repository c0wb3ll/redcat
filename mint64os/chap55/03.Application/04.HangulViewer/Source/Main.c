...
int Main( char* pcArgument ) {
    ...
    if( strlen( pcArgument ) == 0 ) {
        ...
        printf( "ex) exec hanviewer.elf <filename>\n" );
        return 0;
...
// 파일을 찾아서 파일의 크기만큼 버퍼를 할당하고 라인 별 파일 오프셋 버퍼를 할당한 뒤에 파일의 내용을 읽어서 메모리에 저장
BOOL ReadFileToBuffer( const char* pcFileName, TEXTINFO* pstInfo ) {
    ...
    while( 1 ) {
        ...
        if( ( strlen( pstEntry->d_name ) == strlen( pcFileName ) ) && ( memcmp( pstEntry->d_name, pcFileName, strlen( pcFileName ) ) == 0 ) ) {
...
BOOL DrawTextBuffer( QWORD qwWindowID, TEXTINFO* pstInfo ) {
    ...
    DrawRect( qwWindowID, 2, iYOffset, iWidth - 3, WINDOW_TITLEBAR_HEIGHT * 2, RGB( 192, 192, 192 ), TRUE );
    ...
    DrawText( qwWindowID, ( iWidth - iLength * FONT_ENGLISHWIDTH ) / 2, WINDOW_TITLEBAR_HEIGHT + 2, RGB( 0, 0, 0 ), RGB( 192, 192, 192 ), vcBuffer, strlen( vcBuffer ) );

    iYOffset = ( WINDOW_TITLEBAR_HEIGHT * 2 ) + MARGIN;
    DrawRect( qwWindowID, iXOffset, iYOffset, iXOffset + FONT_ENGLISHWIDTH * pstInfo->iColumnCount, iYOffset + FONT_ENGLISHHEIGHT * pstInfo->iRowCount, RGB( 0, 0, 0 ), TRUE );
        ...
        for( i = 0; ( i < iColumnCountToPrint ) && ( iColumnIndex < pstInfo->iColumnCount ); i++ ) {
            ...
            if( bTemp == '\n' ) {
                ...
                if( ( bTemp & 0x80 ) == 0 ) {
                    ...
                    DrawText( qwWindowID, iColumnIndex * FONT_ENGLISHWIDTH + iXOffset, iYOffset + ( j * FONT_ENGLISHHEIGHT ), RGB( 255, 255, 255 ), RGB( 0, 0, 0 ), &bTemp, 1 );
                    iColumnIndex++;

                }
                else if( ( iColumnIndex + 2 ) < pstInfo->iColumnCount ) {

                    DrawText( qwWindowID, iColumnIndex * FONT_ENGLISHWIDTH + iXOffset, iYOffset + ( j* FONT_ENGLISHHEIGHT ), RGB( 255, 255, 255 ), RGB( 0, 0, 0 ), &pstInfo->pbFileBuffer[ i + dwBaseOffset ], 2 );
                    iColumnIndex += 2;
                    i++;
