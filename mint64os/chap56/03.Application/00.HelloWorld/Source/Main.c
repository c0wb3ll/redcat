...
int Main( char* pcArgument ) {
    ...
    qwWindowID = CreateWindow( iMouseX - 10, iMouseY - WINDOW_TITLEBAR_HEIGHT / 2, iWindowWidth, iWindowHeight, WINDOW_FLAGS_DEFAULT, vcTempBuffer );
    ...
    DrawRect( qwWindowID, 10, iY + 8, iWindowWidth - 10, iY + 70, RGB( 255, 255, 255 ), FALSE );
    sprintf( vcTempBuffer, "GUI Event Information[Window ID: 0x%Q, User Mode:%s]", qwWindowID, pcArgument );
    DrawText( qwWindowID, 20, iY, RGB( 255, 255, 255 ), RGB( 0, 0, 0 ), vcTempBuffer, strlen( vcTempBuffer ) );

    SetRectangleData( 10, iY + 80, iWindowWidth - 10, iWindowHeight - 10, &stButtonArea );
    DrawButton( qwWindowID, &stButtonArea, WINDOW_COLOR_BACKGROUND, "User Message Send Button(Up)", RGB( 255, 255, 255 ) );
    ...
    while( 1 ) {
        ...
        case EVENT_MOUSE_MBUTTONUP:
            ...
            DrawText( qwWindowID, 20, iY + 20, RGB( 255, 255, 255 ), WINDOW_COLOR_BACKGROUND, vcTempBuffer, strlen( vcTempBuffer ) );

            sprintf( vcTempBuffer, "Data: X = %d, Y = %d, Button = %X", pstMouseEvent->stPoint.iX, pstMouseEvent->stPoint.iY, pstMouseEvent->bButtonStatus );
            DrawText( qwWindowID, 20, iY + 40, RGB( 255, 255, 255 ), WINDOW_COLOR_BACKGROUND, vcTempBuffer, strlen( vcTempBuffer ) );
            ...
            if( stReceivedEvent.qwType == EVENT_MOUSE_LBUTTONDOWN ) {
                ...
                if( IsInRectangle( &stButtonArea, pstMouseEvent->stPoint.iX, pstMouseEvent->stPoint.iY ) == TRUE ) {
                    ...
                    DrawButton( qwWindowID, &stButtonArea, RGB( 128, 128, 128 ), "User Message Send Button(Down)", RGB( 0, 0, 0 ) );
        ...
        case EVENT_WINDOW_CLOSE:
            ...
            sprintf( vcTempBuffer, "Data: X1 = %d, Y1 = %d, X2 = %d, Y2 = %d", pstWindowEvent->stArea.iX1, pstWindowEvent->stArea.iY1, pstWindowEvent->stArea.iX2, pstWindowEvent->stArea.iY2 );
            DrawText( qwWindowID, 20, iY + 40, RGB( 255, 255, 255 ), WINDOW_COLOR_BACKGROUND, vcTempBuffer, strlen( vcTempBuffer ) );
        ...
        default:
            ...
            sprintf( vcTempBuffer, "Data0 = 0x%Q, Data1 = 0x%Q, Data2 = 0x%Q", stReceivedEvent.vqwData[ 0 ], stReceivedEvent.vqwData[ 1 ], stReceivedEvent.vqwData[ 2 ] );
            DrawText( qwWindowID, 20, iY + 40, RGB( 255, 255, 255 ), WINDOW_COLOR_BACKGROUND, vcTempBuffer, strlen( vcTempBuffer ) );
